...
The results indicate that SVM and the Decision Tree with AdaBoost achieved high predictive accuracy (`{python} round(test_result.loc[2, 'Accuracy'], 4)` and `{python} round(test_result.loc[0, 'Accuracy'], 4)` respectively) making them the most effective models for this classification task. 
In contrast, KNN exhibited comparatively lower performance, achieving an accuracy of `{python} round(test_result.loc[1, 'Accuracy'], 4)`, demonstrating its inferiority relative to the other two models in this context. 
