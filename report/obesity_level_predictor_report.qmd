...
This analysis seeks to use supervised machine learning to predict obesity levels in individuals based on features related to lifestyle habits and physical condition.
...
The results indicate that SVM and the Decision Tree with AdaBoost achieved high predictive accuracy (`{python} round(test_result.loc[1, 'Accuracy'], 4)` and `{python} round(test_result.loc[2, 'Accuracy'], 4)` respectively) making them the most effective models for this classification task. 
In contrast, KNN exhibited comparatively lower performance, achieving an accuracy of `{python} round(test_result.loc[0, 'Accuracy'], 4)`, demonstrating its inferiority relative to the other two models in this context. 
...
Machine learning, a subset of artificial intelligence, has emerged as a promising tool in healthcare, capable of analyzing complex patterns in large datasets [@zhou2022applications]. This brings forth the focus of our research as to why use machine learning as a diagnostic tool for obesity?
...
![Categorical Feature Target Relationship Distribution](../results/figures/categorical_feat_target_relationship.png){#fig-categorical_distribution width=70%}
